...
KIOSK_TOKEN=<optional: gemeinsames Passwort der LAN-Kiosk-Konten und Direkt-Token>
```
...
- `KIOSK_TOKEN` ist optional und dient bestehenden Installationen zugleich als kompatibler
  Direkt-Token. Optional kann ein separates `KIOSK_PASSWORD` nur für die automatischen
  Event-Konten gesetzt werden. Ist keine der beiden Variablen gesetzt, erzeugt der Server beim
