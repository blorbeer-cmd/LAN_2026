...
draftRouter.post('/start', requireGroupRole('admin'), (req, res) => {
  ...
  broadcast(Events.draftChanged, { ...state, started: true }, { groupId: req.group!.id, eventId: row.event_id });
...
draftRouter.post('/cancel', requireGroupRole('admin'), (req, res) => {
  ...
  broadcast(Events.draftChanged, state, { groupId: req.group!.id, eventId: row.event_id });
  res.json(state);
});
