...
function aliveIds(match: TetrisMatch): Set<string> {
  return new Set(match.players.filter((player) => match.states.get(player.id)?.alive).map((player) => player.id));
}
...
function eliminatePlayer(match: TetrisMatch, state: PlayerState, reason: string, sourcePlayerId: string | null = null) {
  ...
  const aliveBefore = aliveIds(match).size;
  state.alive = false;
  state.current = null;
  state.placement = placementForElimination(aliveBefore);
  state.eliminatedAt = Date.now();
  ...
  for (const other of match.states.values()) {
    if (other.alive && other.targetId === state.ref.id) {
      other.targetId = nextArenaTarget(playerOrder, living, other.ref.id, state.ref.id);
    }
  }
...
function spawnNext(match: TetrisMatch, state: PlayerState) {
  ...
  if (collides(state.board, piece)) {
    eliminatePlayer(match, state, 'top-out', state.lastGarbageSourceId);
  } else {
    state.current = piece;
    state.dropAcc = 0;
  }
...
function startLoop(io: Server, match: TetrisMatch) {
  ...
  match.loop = setInterval(() => {
    ...
    for (const bot of match.states.values()) {
      if (!isBotId(bot.ref.id) || !bot.alive || !bot.current) continue;
...
function handleMatchDeparture(io: Server, match: TetrisMatch, playerId: string, excludedSocketId?: string) {
  ...
  if (match.host.id === playerId) {
    ...
    if (nextHost) match.host = nextHost;
  }
...
export function registerTetrisSockets(io: Server): void {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('tetris:input', (payload: { matchId?: string; playerId?: string; action?: InputAction }, ack?: (res: unknown) => void) => {
      ...
      if (!state || isBotId(state.ref.id) || match.socketIds.get(state.ref.id) !== socket.id || !state.alive) return ack?.({ ok: false });
