...
async function verifyArenaLeaveSecurity(authMode: 'legacy' | 'required'): Promise<void> {
  ...
  try {
    ...
    const afterOwnLeave = waitForEvent<{ world: { snakes: Array<{ alive: boolean }> } }>(sockets[0], 'snake:state');
    ...
    const afterHostLeave = waitForEvent<{ host: { id: string }; world: { snakes: Array<{ alive: boolean }> } }>(sockets[2], 'snake:state');
    ...
    const matchEnd = waitForEvent<{ winner: { id: string } | null; reason: string }>(sockets[3], 'snake:match:end');
    sockets[2].close();
    const ended = await matchEnd;
    assert.equal(ended.winner?.id, playerIds[3], `${authMode}: disconnect is an immediate forfeit`);
