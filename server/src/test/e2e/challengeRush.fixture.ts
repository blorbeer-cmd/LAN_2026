...
type ChallengeRushShard = 'scenarios' | 'lifecycle';

const challengeRushShard = process.env.E2E_CHALLENGE_RUSH_SHARD as ChallengeRushShard | undefined;
if (!challengeRushShard || !['scenarios', 'lifecycle'].includes(challengeRushShard)) {
  throw new Error(`Unbekannter Challenge-Rush-Shard: ${challengeRushShard ?? '(fehlt)'}`);
}

function challengeRushTest(
  shard: ChallengeRushShard,
  name: string,
...
challengeRushTest('scenarios', 'Challenge Rush lets a guest leave a running match without ending it for the host', async () => {
...
challengeRushTest('scenarios', 'Challenge Rush unlocks a new lobby immediately after a reconnect rejected past the forfeit grace period', async () => {
