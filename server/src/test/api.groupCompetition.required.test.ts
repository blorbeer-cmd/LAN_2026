...
test('matches, matchmaking draws, tournaments and their aggregates stay isolated across two groups', () => {
  const script = `
    ...
    (async () => {
      ...
      const dave = await register('Competition Dave', 'competition dave secure passphrase');
      ...
      assert.equal((await scoped(app, 'put', '/api/events/' + eventA.body.id + '/participants', alice.cookie, groupA)
        .send({ playerIds: [alice.account.id, bob.account.id] })).status, 200);
      assert.equal((await scoped(app, 'put', '/api/events/' + eventB.body.id + '/participants', carol.cookie, groupB)
      ...
      // A player from another group cannot be smuggled into any competition
      // writer, even though the account exists instance-wide.
      const foreignPlayerMatch = await scoped(app, 'post', '/api/matches', alice.cookie, groupA).send({
        gameId: gameA.body.id,
        teams: [{ playerIds: [alice.account.id] }, { playerIds: [carol.account.id] }],
      });
      assert.equal(foreignPlayerMatch.status, 404);
      const foreignPlayerDraw = await scoped(app, 'post', '/api/matchmaking', alice.cookie, groupA).send({
        gameId: gameA.body.id,
        playerIds: [alice.account.id, carol.account.id],
      });
      assert.equal(foreignPlayerDraw.status, 404);
      const foreignPlayerTournament = await scoped(app, 'post', '/api/tournaments', alice.cookie, groupA).send({
        gameId: gameA.body.id,
        format: 'single_elimination',
        teams: [{ playerIds: [alice.account.id] }, { playerIds: [carol.account.id] }],
      });
      ...
      // The global tracking event belongs to A right now. B must not adopt
      // that event id into rows carrying group B.
      const foreignTrackingMatch = await scoped(app, 'post', '/api/matches', carol.cookie, groupB).send({
        gameId: gameB.body.id,
        teams: [{ playerIds: [carol.account.id] }, { playerIds: [dave.account.id] }],
      });
      assert.equal(foreignTrackingMatch.status, 409);
      const foreignTrackingDraw = await scoped(app, 'post', '/api/matchmaking', carol.cookie, groupB).send({
        gameId: gameB.body.id,
        playerIds: [carol.account.id, dave.account.id],
      });
      assert.equal(foreignTrackingDraw.status, 409);
      const foreignTrackingTournament = await scoped(app, 'post', '/api/tournaments', carol.cookie, groupB).send({
        gameId: gameB.body.id,
        format: 'single_elimination',
        teams: [{ playerIds: [carol.account.id] }, { playerIds: [dave.account.id] }],
      });
      assert.equal(foreignTrackingTournament.status, 409);
      ...
      // The event roster is a historical snapshot: removing Dave's active
      // membership must not invalidate competition records for this event.
      assert.equal((await request(app)
        .delete('/api/groups/' + groupB + '/members/' + dave.account.id)
        .set('Cookie', carol.cookie)).status, 204);
      ...
      const tournamentB = await scoped(app, 'post', '/api/tournaments', carol.cookie, groupB).send({
        gameId: gameB.body.id,
        format: 'single_elimination',
        teams: [{ playerIds: [carol.account.id] }, { playerIds: [dave.account.id] }],
      });
      ...
      // Instance admin is deliberately insufficient: Alice is only a
      // regular member in B, while Carol is B's non-instance Owner.
      await addMember(carol, groupB, alice);
      const instanceAdminDelete = await scoped(app, 'delete', '/api/tournaments/' + tournamentB.body.id, alice.cookie, groupB);
      assert.equal(instanceAdminDelete.status, 403);
      const ownerDelete = await scoped(app, 'delete', '/api/tournaments/' + tournamentB.body.id, carol.cookie, groupB);
      ...
      const memberDelete = await scoped(app, 'delete', '/api/matches/' + matchA.body.id, bob.cookie, groupA);
      assert.equal(memberDelete.status, 403);

      // Leaderboards only reflect each group's own match.
      ...
      const carolInB = leaderboardB.body.standings.find((s) => s.playerId === carol.account.id);
      assert.equal(aliceInA.matchesPlayed, 1);
      assert.equal(carolInB.matchesPlayed, 1);
      ...
      // A group admin (without relying on instance-admin state) may delete
      // group-owned competition history; Owners remain allowed as well.
      ...
      const groupAdminDelete = await scoped(app, 'delete', '/api/tournaments/' + tournamentA.body.id, bob.cookie, groupA);
      assert.equal(groupAdminDelete.status, 204);
      const groupOwnerDelete = await scoped(app, 'delete', '/api/matches/' + matchA.body.id, alice.cookie, groupA);
