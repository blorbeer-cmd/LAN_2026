...
test('migration 85 enables Arcade for existing general events without changing LAN snapshots', () => {
  const dbFile = makeTempDbPath('general-event-arcade');
  runMigrations(dbFile);
  ...
  fixture
    .prepare(
      `INSERT INTO events
         (id, name, starts_at, ends_at, group_id, status, visibility_scope, event_type_key, preset_version)
       VALUES (?, ?, ?, ?, 'default-group', 'published', 'participants', 'general', 1)`,
    )
    .run('existing-general-event', 'Existing General Event', now, now + 60_000);
  const insertFeature = fixture.prepare(
    `INSERT INTO event_features (event_id, feature_key, enabled, changed_at, changed_by)
     VALUES (?, ?, ?, ?, NULL)`,
  );
  for (const featureKey of EVENT_FEATURE_KEYS) {
    insertFeature.run('existing-general-event', featureKey, featureKey === 'arcade' ? 0 : 1, now);
  }
  fixture.prepare('DELETE FROM schema_migrations WHERE version = 85').run();
  fixture.close();
  ...
  assert.deepEqual(
    migrated
      ...
      .get('existing-general-event'),
    { presetVersion: 2 },
  );
  assert.deepEqual(
    migrated
      .prepare('SELECT enabled FROM event_features WHERE event_id = ? AND feature_key = ?')
      .get('existing-general-event', 'arcade'),
    { enabled: 1 },
  );
  assert.deepEqual(
    migrated
      .prepare('SELECT enabled FROM event_features WHERE event_id = ? AND feature_key = ?')
      .get('instance-base-event', 'arcade'),
    { enabled: 1 },
  );
  migrated.close();
  fs.rmSync(path.dirname(dbFile), { recursive: true, force: true });
