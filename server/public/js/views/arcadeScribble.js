  ...
  return `
    <div class="card stack">
      <div class="row-between" style="gap:var(--space-3);">
        ...
        <div class="row" style="gap:var(--space-2);">${currentPlayerMayUseArcadeAi() ? `<button type="button" class="btn btn-sm btn-equal" id="scribble-bot" ${lobby || match || noMe ? 'disabled' : ''}>Gegen KI</button>` : ''}<button type="button" class="btn btn-primary btn-sm btn-equal" id="scribble-create" ${lobby || match || noMe ? 'disabled' : ''}>Lobby öffnen</button></div>
      </div>
      ${arcadeInfoGridHtml([
