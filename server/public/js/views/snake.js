...
export function renderSnakeLobbyCard() {
  ...
  return `<div class="card stack"><div class="row-between" style="gap:var(--space-3);"><strong>Snake-Lobby</strong>
    <div class="row" style="gap:var(--space-2);">${isAdmin() ? `<button type="button" class="btn btn-sm btn-equal" id="snake-bot" ${match || noMe ? 'disabled' : ''}>Gegen KI</button>` : ''}<button type="button" class="btn btn-primary btn-sm btn-equal" id="snake-create" ${match || noMe ? 'disabled' : ''}>Lobby öffnen</button></div></div>
    ${arcadeInfoGridHtml([
