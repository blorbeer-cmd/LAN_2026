...
export function renderBlobbyLobbyCard() {
  ...
  return `<div class="card stack"><div class="row-between" style="gap:var(--space-3);"><strong>Blobby-Volley-Lobby</strong>
    <div class="row" style="gap:var(--space-2);"><button type="button" class="btn btn-sm btn-equal" id="blobby-bot" ${lobby || match || noMe ? 'disabled' : ''}>Gegen KI</button><button type="button" class="btn btn-primary btn-sm btn-equal" id="blobby-create" ${lobby || match || noMe ? 'disabled' : ''}>Lobby öffnen</button></div></div>
    ${arcadeInfoGridHtml([
