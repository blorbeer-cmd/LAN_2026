...
export function renderBlobbyLobbyCard() {
  ...
  return `<div class="card stack"><div class="row-between" style="gap:var(--space-3);"><strong>Blobby-Volley-Lobby</strong>
    <div class="row" style="gap:var(--space-2);">${isAdmin() ? `<button type="button" class="btn btn-sm btn-equal" id="blobby-bot" ${match || noMe ? 'disabled' : ''}>Gegen KI</button>` : ''}<button type="button" class="btn btn-primary btn-sm btn-equal" id="blobby-create" ${match || noMe ? 'disabled' : ''}>Lobby öffnen</button></div></div>
    ${arcadeInfoGridHtml([
