...
test('general events keep core and planning areas while hiding LAN-specific routes', () => {
  for (const view of ['home', 'profile', 'events', 'broadcast', 'foodOrders', 'checklist', 'arrivals', 'music', 'seating']) {
    assert.equal(viewIsEnabledForEvent(view, generalEvent), true, view);
  }
  for (const view of ['matchmaking', 'tournaments', 'votes', 'gameCatalog', 'leaderboard', 'kiosk', 'arcade', 'tetris']) {
