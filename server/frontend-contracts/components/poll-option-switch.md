...
Markup und Wertauslesung: `public/js/views/eventPolls.js` (`optionRowHtml`, `optionValuesFromForm`). Darstellung: `public/css/style.css` (`.event-poll-option-active`).
...
Die Spur ist 44 × 24 px aus `--space-*`-Tokens; der Knauf ist 16 px groß. Der Text „Option aktiv (wählbar)“ steht direkt neben dem Schalter. Neue Optionen starten eingeschaltet, gespeicherte Optionen spiegeln ihren aktiven Zustand. Mindestens eine Option bleibt aktiv.
...
Der native Checkbox-Input erhält `role='switch'` und seinen Namen über das umschließende Label. Leertaste schaltet um; `:focus-visible` zeigt einen Fokusrahmen. Der Zustand bleibt als natives `checked` lesbar.
