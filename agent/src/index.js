...
function start(configPath) {
  ...
  const desktopShortcutPaths = [
    path.join(desktopDir, 'Respawn-Agent Steuerung.lnk'),
    path.join(desktopDir, 'Respawn-Agent Steuerung.url'),
  ];
  ...
  const controlServer = createControlServer({
    ...
    uninstall: () => {
      scheduleUninstall({ installDir, startupShortcutPath: shortcutPath, desktopShortcutPaths });
      if (trayProcess) {
        try {
          trayProcess.kill();
        } catch {
