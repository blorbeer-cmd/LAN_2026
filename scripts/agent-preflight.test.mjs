...
test("names both tooling suites for root and infrastructure work", () => {
  for (const scope of ["root", "infra"]) {
    const result = runPreflight(["--scope", scope]);
    assert.equal(result.status, 0, result.stderr);
    assert.match(
      result.stdout,
      /node --test scripts\/agent-preflight\.test\.mjs scripts\/pr-completion\.test\.mjs/,
    );
  }
});
